...
export const levelTwoLessons: readonly CurriculumLesson[] = [
  {
    ...
    guidedExercises: [
      ...
      { id: "exercise.transpose-functions", title: "Move functions, not memorized names", purpose: "Connect transposition to stable harmonic roles.", instructions: ["Write I-IV-V-vi in G.", "Move each root up five semitones to C and name qualities.", "Move G roots up seven semitones to D and verify the result."], successCriteria: ["Root distances remain consistent.", "I, IV, V stay major and vi stays minor.", "Chord names match the selected keys."], reduceDifficultyWhen: ["Transpose I-IV-V."], increaseDifficultyWhen: ["Choose a fourth key and select contrasting voicings."], relatedSkills: ["transposition", "Roman numerals", "triad quality"] }
    ],
    commonMistakes: [
  ...
  },
  {
    ...
    guidedExercises: [
      ...
      { id: "exercise.blues-articulation", title: "One articulation at a time", purpose: "Keep expressive technique tied to pitch and rhythm.", instructions: ["Play one two-note phrase with picked attacks.", "Repeat with a hammer-on or slide.", "Add vibrato to the final held note.", "Use a bend after matching its target pitch first."], successCriteria: ["Rhythm remains unchanged between versions.", "The target pitch is named.", "The hand remains comfortable."], reduceDifficultyWhen: ["Use slide and vibrato."], increaseDifficultyWhen: ["Choose articulation based on phrase direction without prompts."], relatedSkills: ["articulation", "pitch", "expression"] }
    ],
    commonMistakes: [
      { id: "mistake.blues-box-run", symptom: "Phrases keep running up and down the whole box.", likelyCause: "Pattern recall is replacing phrase choice.", adjustment: "Limit to three notes, repeat one rhythm, and require a full-beat rest." },
  ...
  },
  {
    ...
    contentBlocks: [
      ...
      ], explanation: "Learn the open and fret-12 anchors, then B-C and E-F adjacency, then fill whole-step gaps. Use random prompts instead of repeating one memorized order.", accessibilityDescription: "Strings 6 and 5 natural-note map from open through fret 12. String 6: E F G A B C D E. String 5: A B C D E F G A." },
      ...
      ...stages("fretboard-five-seconds", "natural-note and octave retrieval", ["Trace string 6 natural notes from E using half-step pairs.", "Trace string 5 from A without restarting the alphabet.", "Observe the two-strings-thinner, two-frets-higher octave rule."], ["Use shuffled natural-note prompts on string 6, then string 5.", "Allow ten seconds and speak the nearest anchor used.", "After each root, derive its octave and verify the pitch class."], ["Reduce response time to seven seconds and hide the map until after each answer.", "Mix strings 6 and 5 prompts.", "Use one chromatic root such as B flat to check that the octave shape transfers."], ["Find prompted natural notes on string 6 or 5 within five seconds.", "Derive and play nearby octaves without a diagram.", "Perform or create a riff using named roots in two registers."], ["Natural-note locations are correct within five seconds.", "Octave derivation preserves note identity.", "The riff uses named roots, not anonymous shapes."]),
      { id: "fretboard-reflection", type: "reflection", heading: "Record the anchor and the answer", prompt: "Which note-string prompt was slow, which nearby anchor solved it, and which octave route will you retrieve next session?", fieldLabel: "Fretboard retrieval observation", placeholder: "String 5 F was slow; E at fret 7 plus one half step found fret 8. Next retrieve its string 3 fret 10 octave." }
    ],
    guidedExercises: [
    ...
    commonMistakes: [
      { id: "mistake.fretboard-count", symptom: "Notes are found by counting from the open string.", likelyCause: "No interior anchors have been retained.", adjustment: "Use fret 5, 7, or 8 natural anchors and state the shortest route." },
  ...
  },
  {
    ...
    contentBlocks: [
      ...
      ...stages("major-melody-development", "an eight-bar developed melody", ["Sing degrees 1-7-1 and identify tonic pull.", "Observe the motive, sequence, climax, and resolution in the model.", "Compare G-major and C-major root positions without treating fret numbers as note identity."], ["Play each scale one octave with named degrees.", "Learn the model in two-bar phrases.", "Write a two-bar motive and sequence it one degree higher."], ["Use degree numbers and form labels for the model melody.", "Choose a climax note before composing bars 5-6.", "Remove the scale diagram and verify notes afterward."], ["Play either movable major pattern from a named root.", "Perform an original eight-bar melody with repetition, sequence, climax, and tonic resolution.", "Explain its contour and relative-minor relationship."], ["Scale degrees and note names match the selected key.", "The melody has audible development and a clear climax.", "The ending confirms tonic or explains why it avoids tonic."]),
    ...
    guidedExercises: [
      { id: "exercise.major-thirds", title: "Sequence in thirds without racing", purpose: "Hear non-adjacent scale relationships while retaining degree names.", instructions: ["Play 1-3, 2-4, 3-5, 4-6 at 55 BPM.", "Sing the lower degree before each pair.", "Return down through the same sequence."], successCriteria: ["Notes belong to the key.", "Degree names remain accurate.", "Tone and pulse remain lyrical."], reduceDifficultyWhen: ["Use pairs 1-3 and 2-4."], increaseDifficultyWhen: ["Transfer the sequence to the second position."], relatedSkills: ["major scale", "thirds", "ear training"] },
  ...
  },
  {
    ...
    objective: "Maintain a sixteenth-note motion grid, perform syncopation, ties, ghost strokes, muting, accents, straight and shuffle feels, and sustain a planned groove for two minutes.",
    ...
    contentBlocks: [
      ...
      ], explanation: "The second event of each beat lands on the final triplet subdivision. A shuffle needs a stable triplet reference, not random uneven timing.", accessibilityDescription: "Four shuffle beats subdivided 1-trip-let through 4-trip-let, with downstrokes on numbers, holds on trip, and upstrokes on let." },
    ...
    guidedExercises: [
      ...
      { id: "exercise.groove-feels", title: "Straight and shuffle comparison", purpose: "Hear feel as subdivision organization.", instructions: ["Play straight eighths at 60 BPM.", "Count triplets and play first-last shuffle events.", "Alternate two measures of each while keeping the beat unchanged."], successCriteria: ["Beat speed remains unchanged.", "Straight events divide evenly.", "Shuffle events align with triplet first and last slots."], reduceDifficultyWhen: ["Clap one beat several times."], increaseDifficultyWhen: ["Apply each feel to the progression."], relatedSkills: ["straight feel", "shuffle", "subdivision"] }
    ],
    commonMistakes: [
      { id: "mistake.groove-stop", symptom: "The hand stops on rests or ties.", likelyCause: "Motion exists only for sounded attacks.", adjustment: "Mute all strings and preserve the complete down-up grid before restoring sound." },
      ...
      { id: "mistake.groove-shuffle", symptom: "Shuffle is uneven and changes each beat.", likelyCause: "No triplet subdivision anchors the delayed upstroke.", adjustment: "Count triplets and place events on the first and last slot." }
    ],
    knowledgeChecks: [
  ...
  },
  {
    ...
    guidedExercises: [
      ...
      { id: "exercise.triad-nearest", title: "Choose the nearest inversion", purpose: "Create a musical second part with its own role.", instructions: ["Play the first chord on strings 3-2-1.", "List all inversions of the next chord.", "Choose the voicing with smallest total movement.", "Track one voice aloud through the pair."], successCriteria: ["Chord identity remains correct.", "At least one voice moves by step or remains common.", "The second part occupies a distinct register."], reduceDifficultyWhen: ["Compare two inversion choices."], increaseDifficultyWhen: ["Arrange four chords and add independent rhythm."], relatedSkills: ["inversions", "voice leading", "arrangement"] }
    ],
    commonMistakes: [
      ...
      { id: "mistake.triad-inversion", symptom: "Each rearrangement gets called a new chord quality.", likelyCause: "Lowest note and chord identity are being conflated.", adjustment: "Collect all three notes first; then name inversion from the lowest chord tone." },
    ...
    knowledgeChecks: [
      ...
      { id: "check.triad-voice-leading", prompt: "What does efficient voice leading prioritize?", options: ["Small planned motion and common tones", "Always the lowest available fret", "All voices jumping in parallel"], correctAnswer: "Small planned motion and common tones", explanation: "Voice leading considers each note's path between chords." }
    ],
    masteryCriteria: [
    ...
    optionalExtension: "Write an independent rhythm for the triad part that answers the original accompaniment.",
  ...
  },
  {
    ...
    whyItMatters: "A lead sheet shares musical decisions without prescribing each note. Functional numbers preserve relationships when singer range, capo position, or ensemble register requires a new key.",
    estimatedMinutes: 120,
    ...
    guidedExercises: [
      ...
      { id: "exercise.chart-transpose", title: "Function-preserving transposition", purpose: "Move relationships before naming chords.", instructions: ["Write the source key scale degrees.", "Label each chord with function and quality.", "Write target-key roots for the same degrees.", "Verify each root by interval before playing."], successCriteria: ["Function order is unchanged.", "Chord qualities are preserved.", "The performed target key matches the written chart."], reduceDifficultyWhen: ["Transpose I-IV-V."], increaseDifficultyWhen: ["Add slash bass or capo-shape notation."], relatedSkills: ["transposition", "Roman numerals", "Nashville numbers"] }
    ],
    commonMistakes: [
      { id: "mistake.chart-chords-only", symptom: "Chord names are correct but repeats and section order fail.", likelyCause: "The chart was scanned vertically as a chord list.", adjustment: "Trace the form and first chord of each section before reading measures." },
  ...
  {
    id: "lesson.level-two-band-songwriting-project",
    unitId: "unit.level-two-band-songwriting-project",
    order: 1,
    title: "Build, rehearse, and assess a complete Level 2 performance",
    objective: "Plan and perform a two-to-four-minute original piece with a readable chart, contrasting sections, rhythm and lead roles, one improvised passage, a short ear transcription, functional analysis, and self-evaluation from what you heard.",
    whyItMatters: "A complete piece tests whether technique, time, fretboard knowledge, harmony, melody, listening, and recovery remain available while musical form keeps moving. The project values clear decisions and communication over polish without understanding.",
    estimatedMinutes: 240,
    priorKnowledge: ["Movable harmony", "Pentatonic and major-scale phrasing", "Fretboard roots and octaves", "Rhythm-guitar vocabulary", "Triads and voice leading", "Lead-sheet reading and transposition"],
    contentBlocks: [
      { id: "level-two-project-brief", type: "text", heading: "One complete piece can show several kinds of musicianship", paragraphs: ["Create an original piece lasting two to four minutes. Include at least two contrasting sections, a planned opening and ending, one accompaniment role, one melodic or lead role, and an improvised passage whose length is fixed in the chart.", "Prepare three contrasting studies before choosing the final piece: a groove-led accompaniment, a melody-led arrangement, and a form-led song sketch. These are short rehearsals, not three polished recordings. Select the one that gives you the clearest musical result and note what the other two taught you.", "You may perform each role yourself, use a looper or backing track you control, or rehearse with other musicians. Recording on your own device can support reflection but is optional; FretGarden does not require or accept an upload for this project."] },
      { id: "level-two-project-chart", type: "lead-sheet", heading: "Original project model: Lantern Lines", songTitle: "Lantern Lines", key: "G", meter: "4/4", tempo: 76, capo: 0, sections: [
        { name: "Intro", repeatCount: 1, measures: [{ chord: "G", cue: "root-octave riff; quiet", beats: 4 }, { chord: "Cadd9", cue: "answer and hold", beats: 4 }] },
        { name: "Verse", repeatCount: 2, measures: [{ chord: "G", cue: "sixteenth ghost-stroke groove", beats: 4 }, { chord: "D/F#", cue: "keep bass connected", beats: 4 }, { chord: "Em", cue: "leave beat 4 open", beats: 4 }, { chord: "C", cue: "fill on 4-and", beats: 4 }] },
        { name: "Chorus", repeatCount: 2, measures: [{ chord: "C", cue: "open eighths; stronger", beats: 4 }, { chord: "G", cue: "melody answer", beats: 4 }, { chord: "D", cue: "build", beats: 4 }, { chord: "Em", cue: "hold then breathe", beats: 4 }] },
        { name: "Improvisation", repeatCount: 2, measures: [{ chord: "Em", cue: "E minor pentatonic; phrase", beats: 4 }, { chord: "C", cue: "target E or G", beats: 4 }, { chord: "G", cue: "answer with space", beats: 4 }, { chord: "D", cue: "resolve or hand back", beats: 4 }] },
        { name: "Outro", repeatCount: 1, measures: [{ chord: "C", cue: "intro answer; quieter", beats: 4 }, { chord: "G", cue: "shared final cutoff", beats: 4 }] }
      ], explanation: "The model separates rhythm, melodic, and improvised responsibilities while keeping one coherent form. Use it to study project scope, then create a different progression, riff, melody, and title for the independent submission.", accessibilityDescription: "Original G-major model in four-four at 76 BPM: two-bar intro, eight-bar repeated verse, eight-bar repeated chorus, eight-bar fixed improvisation over E minor C G D, and two-bar outro." },
      { id: "level-two-role-map", type: "progression-chart", heading: "Assign a job to every section", key: "G", meter: "4/4", measures: [
        { label: "Intro", chord: "G-Cadd9", romanNumeral: "I-IV", nashvilleNumber: "1-4", beats: 4 },
        { label: "Verse", chord: "G-D/F#-Em-C", romanNumeral: "I-V6-vi-IV", nashvilleNumber: "1-5/7-6m-4", beats: 4 },
        { label: "Chorus", chord: "C-G-D-Em", romanNumeral: "IV-I-V-vi", nashvilleNumber: "4-1-5-6m", beats: 4 },
        { label: "Solo", chord: "Em-C-G-D", romanNumeral: "vi-IV-I-V", nashvilleNumber: "6m-4-1-5", beats: 4 },
        { label: "Outro", chord: "C-G", romanNumeral: "IV-I", nashvilleNumber: "4-1", beats: 4 }
      ], explanation: "Each row summarizes a section, not one literal measure. Mark who owns pulse, low register, chord color, melody, fills, transitions, and the final cutoff; silence is also an arrangement choice.", accessibilityDescription: "Section function map for Lantern Lines: intro I-IV, verse I-V first inversion-vi-IV, chorus IV-I-V-vi, solo vi-IV-I-V, outro IV-I." },
      { id: "level-two-ear-transcription", type: "tablature", heading: "Model a four-note hear-sing-find-write cycle", tempo: 64, events: [
        { count: "1", notes: [{ string: 5, fret: 2 }], duration: "quarter", rest: false },
        { count: "2", notes: [{ string: 4, fret: 2 }], duration: "quarter", rest: false },
        { count: "3", notes: [{ string: 4, fret: 4 }], duration: "quarter", rest: false },
        { count: "4", notes: [], duration: "quarter", rest: true },
        { count: "1", notes: [{ string: 4, fret: 2 }], duration: "quarter", rest: false },
        { count: "2", notes: [{ string: 5, fret: 2 }], duration: "quarter", rest: false },
        { count: "3", notes: [{ string: 5, fret: 5 }], duration: "half", rest: false }
      ], explanation: "Have another person, a shuffled note source, or a recording on your own device present a short riff. Hear it repeatedly, sing its contour, find one note at a time, then write rhythm and frets before checking. If no sound source is available, cover this model, reconstruct it from a delayed sung memory, and compare afterward.", accessibilityDescription: "Two-bar B-minor phrase: B E F sharp, quarter-note rest, then E B D with the final D held for two beats." },
      ...stages("level-two-integration", "a complete Level 2 piece", ["Trace the model form and identify the job of each guitar part.", "Hear how accompaniment density changes between verse and chorus.", "Observe a transcription cycle that delays notation until after hearing and singing."], ["Sketch three contrasting 30-60 second studies: groove-led, melody-led, and form-led.", "Choose one study and draft a complete lead sheet with section, repeat, cue, and ending information.", "Rehearse rhythm and lead roles separately before combining them."], ["Use the chart and a short transition checklist.", "Remove written scale diagrams during the fixed improvisation section.", "Run the entire form after mistakes without restarting, then repair the weak transition."], ["Perform the original two-to-four-minute piece from its chart.", "Complete the short ear transcription without viewing an answer first.", "Submit the chart, functional analysis, performance checklist, and self-evaluation from what you heard."], ["Pulse, form, and section lengths remain recoverable from beginning to ending.", "Rhythm, melody, and improvisation serve distinct musical roles.", "The chart and analysis let another musician understand the intended piece.", "Reflection cites observable musical details and a specific next exercise."]),
      { id: "level-two-project-reflection", type: "reflection", heading: "Evaluate decisions, not personality", prompt: "Record the key, meter, form, progression functions, chosen scale or note collection, strongest transition, least stable moment, recovery note, and one narrow next exercise. Also name why the selected study worked better than the other two.", fieldLabel: "Level 2 project evaluation", placeholder: "G major, 4/4, A-B-A-solo-B. The chorus entry stayed in time; the solo ending over V was late. Next: loop the final two solo bars into the chorus at 60 BPM." }
    ],
    guidedExercises: [
      { id: "exercise.level-two-three-sketches", title: "Three contrasting studies", purpose: "Separate exploration from final-project polishing.", instructions: ["Make a groove-led sketch with a clear mute or syncopation choice.", "Make a melody-led sketch with a developed motive.", "Make a form-led sketch with at least two contrasting sections.", "Choose one using the published success criteria."], successCriteria: ["Each sketch has a different primary musical problem.", "The choice uses musical results, not familiarity alone.", "Useful material from unselected sketches is recorded."], reduceDifficultyWhen: ["Limit each sketch to 30 seconds and two chords."], increaseDifficultyWhen: ["Transpose one sketch or arrange it for a second guitar role."], relatedSkills: ["composition", "selection", "reflection"] },
      { id: "exercise.level-two-performance-run", title: "No-restart project run", purpose: "Test complete-form recovery inside the piece.", instructions: ["Count in and perform the entire chart without restarting.", "Mark lost form, pulse breaks, unclear roles, and failed transitions.", "Choose the highest-impact issue and isolate it for five minutes.", "Run the complete form again."], successCriteria: ["The ending arrives after the intended form.", "Mistakes do not erase the pulse or next section.", "The second run tests one planned repair."], reduceDifficultyWhen: ["Run intro, one verse, one chorus, and outro."], increaseDifficultyWhen: ["Change tempo, register, or ensemble context without changing form."], relatedSkills: ["performance", "recovery", "form"] },
      { id: "exercise.level-two-transcription", title: "Hear, sing, find, write, verify", purpose: "Listen before using a visible fret pattern.", instructions: ["Listen to a two-to-four-note source at least three times without the guitar.", "Sing the contour and tap the rhythm.", "Find one pitch at a time and name known notes or intervals.", "Write rhythm and fret locations, then verify against the source."], successCriteria: ["The first search follows the heard contour.", "Rhythm and pitch are written separately when needed.", "Corrections are marked instead of erased from the learning record."], reduceDifficultyWhen: ["Use two notes with quarter-note rhythm."], increaseDifficultyWhen: ["Use a changed key, register, or starting string."], relatedSkills: ["ear training", "transcription", "fretboard"] }
    ],
    commonMistakes: [
      { id: "mistake.level-two-overload", symptom: "The piece contains many recent devices but no clear musical center.", likelyCause: "Feature count has replaced arrangement purpose.", adjustment: "Give each section one primary job and remove anything that competes with it." },
      { id: "mistake.level-two-loop", symptom: "A strong loop never becomes a complete piece.", likelyCause: "Contrast, transitions, and ending were postponed.", adjustment: "Write the form and ending first, then assign existing material to sections." },
      { id: "mistake.level-two-restart", symptom: "Errors send the performance back to bar one.", likelyCause: "Isolated accuracy has not been connected to form recovery.", adjustment: "Practice entering at each section and run the chart with a no-restart rule." },
      { id: "mistake.level-two-analysis-after", symptom: "Roman numerals and scale labels are added after composition but cannot explain a decision.", likelyCause: "Analysis is being treated as decoration.", adjustment: "Use function to justify one contrast, target note, transposition, or ending choice." }
    ],
    knowledgeChecks: [
      { id: "check.level-two-evidence", prompt: "Which result best shows integration?", options: ["A complete recoverable performance plus chart, analysis, and reflection", "A flawless isolated scale", "A list of all chord shapes learned"], correctAnswer: "A complete recoverable performance plus chart, analysis, and reflection", explanation: "Integration asks skills to stay available inside a complete musical form." },
      { id: "check.level-two-arrangement", prompt: "What should a second guitar role usually do?", options: ["Occupy a complementary register, rhythm, or function", "Double the first part all the time", "Play louder than the first part"], correctAnswer: "Occupy a complementary register, rhythm, or function", explanation: "Complementary roles preserve clarity and make the ensemble easier to hear." },
      { id: "check.level-two-transcription", prompt: "What comes before searching the fretboard in the transcription cycle?", options: ["Hear the source and sing or tap its contour and rhythm", "Read a completed tab", "Choose a familiar scale shape at random"], correctAnswer: "Hear the source and sing or tap its contour and rhythm", explanation: "Delayed visual support keeps the task focused on listening before copying." }
    ],
    masteryCriteria: [
      { id: "mastery.level-two-chart", description: "Create a readable lead sheet with key, meter, form, repeats, cues, and ending.", verification: "performance-checklist", required: true },
      { id: "mastery.level-two-performance", description: "Perform a two-to-four-minute original piece with contrasting sections, stable pulse, and recovery.", verification: "performance-checklist", required: true },
      { id: "mastery.level-two-roles", description: "Demonstrate accompaniment, melodic or lead, and fixed-length improvised roles independently or with an ensemble-equivalent setup.", verification: "performance-checklist", required: true },
      { id: "mastery.level-two-transcription", description: "Transcribe a short heard riff through a delayed hear-sing-find-write cycle.", verification: "guided-self-check", required: true },
      { id: "mastery.level-two-analysis", description: "Analyze key, meter, form, progression function, note collection, and one arrangement decision.", verification: "reflection", required: true },
      { id: "mastery.level-two-reflection", description: "Cite observable performance details and define one targeted next exercise.", verification: "reflection", required: true }
    ],
    reviewRecommendation: "At the next session, retrieve the chart and weakest transition before a complete run. After one week, perform the piece in a changed tempo, key, register, or ensemble context and note what transfers.",
    optionalExtension: "Prepare a second arrangement that changes instrumentation or register while preserving form and harmonic function."
  }
];
...
export const levelTwoReviewPlans: readonly CurriculumReviewPlan[] = [
  ...
  { id: "review.level-two-band-songwriting-project", unitId: "unit.level-two-band-songwriting-project", immediateReview: ["Record the strongest moment, weakest transition, and exact next exercise."], nextSessionReview: ["Retrieve the chart and repair the weakest transition before a complete no-restart run."], oneWeekReview: ["Perform in a changed tempo, key, register, or ensemble context."], longTermReview: ["Revisit the project after later levels and document how arrangement, harmony, and improvisation choices changed."] }
];
